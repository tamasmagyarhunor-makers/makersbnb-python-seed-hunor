...
def test_instantiation():
    listing = Listing(1, "house", "nice house", 100, 'example.jpeg', 1)
    assert listing.id == 1
    assert listing.name == "house"
    assert listing.description == "nice house"
    assert listing.price == 100
    assert listing.image == 'example.jpeg'
...
def test_formatting():
    listing = Listing(1, "house", "nice house", 100, 'example.jpeg', 1)
    assert str(listing) == "Listing(1, house, nice house, 100, example.jpeg, 1)"

"""
test that Listings with identical attributes are equal
"""
def test_equality():
    listing1 = Listing(1, "house", "nice house", 100, 'example.jpeg', 1)
    listing2 = Listing(1, "house", "nice house", 100, 'example.jpeg', 1)
