...
class BookingRequestRepository:
    ...
    # Insert and get the new booking's ID (Postgres syntax)
        result = self._connection.execute(
        'INSERT INTO booking_requests (start_date, end_date, space_id, user_id) VALUES (%s, %s, %s, %s) RETURNING id',
        [start_date, end_date, space_id, user_id]
        )
        new_id = result[0]['id']

    # Fetch and return the new booking request object
        row = self._connection.execute('SELECT * FROM booking_requests WHERE id = %s', [new_id])[0]
        return BookingRequest(row['id'], row['start_date'], row['end_date'], row['space_id'], row['user_id'])
